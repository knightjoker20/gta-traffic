...
const importSource =
  ...
  // ── Spawn name bulk install / uninstall ─────────────
  async function onSpawnNameApply() {
    const textarea   = el("vlSpawnNameInput");
    const toggle     = el("vlSpawnInstallToggle");
    const resultHost = el("vlSpawnResult");
    const applyBtn   = el("vlSpawnApplyBtn");
    if (!textarea || !toggle || !resultHost) return;

    const installing = toggle.checked;
    const raw = textarea.value;

    // Accept names separated by newlines, commas, or whitespace
    const names = raw
      .split(/[\n\r,\s]+/)
      .map(n => store.normalizeId(n.trim()))
      .filter(Boolean);

    if (!names.length) {
      resultHost.innerHTML = `<div class="vl-scan-warning">No model names found — paste at least one spawn name.</div>`;
      return;
    }
    ...
    for (const id of names) {
      ...
      if (!vehicle) {
        ...
        // Installing an unknown model → create a minimal library entry
        vehicle = store.createVehicleSkeleton(id);
        vehicle.id = id;
        created.push(id);
      }

      vehicle.custom = vehicle.custom || {};
      vehicle.custom.installed    = installing;
      vehicle.custom.installDate  = installing ? (vehicle.custom.installDate || today) : "";
      vehicle.custom.installSource = installing ? "Spawn name list" : "";
      vehicle.custom.lastInstallScanAt = new Date().toISOString();
      vehicle.updatedAt = new Date().toISOString();
    ...
    const verb = installing ? "installed" : "uninstalled";
    resultHost.innerHTML = `
      <div class="vl-scan-summary">
  ...
  if (window.vehicleCloud) {
    try {
      ...
      if (cloudData.vehicles.length > 0) {
        // Merge vehicles: cloud wins on metadata, but preserve local-only
        // scan fields so the RPF scanner's installed flags aren't wiped.
        const localMap = new Map(localVehicles.map(v => [v.id, v]));
        vehicles = cloudData.vehicles.map(cloudVehicle => {
          const local = localMap.get(cloudVehicle.id);
          if (!local?.custom) return cloudVehicle;
          return {
            ...cloudVehicle,
            custom: {
              ...cloudVehicle.custom,
              // These fields are written locally by the RPF scanner and
              // install/uninstall toggles — the cloud has no record of them.
              installed:            local.custom.installed            ?? cloudVehicle.custom?.installed ?? false,
              installDate:          local.custom.installDate          || cloudVehicle.custom?.installDate          || null,
              lastInstallScanFiles: local.custom.lastInstallScanFiles || cloudVehicle.custom?.lastInstallScanFiles || [],
              lastInstallScanAt:    local.custom.lastInstallScanAt    || cloudVehicle.custom?.lastInstallScanAt    || null,
              installSource:        local.custom.installSource        || cloudVehicle.custom?.installSource        || null,
            }
          };
        });
  ...
  return `
    <article class="vl-vehicle-card ${isSelected ? "vl-bulk-selected" : ""}" data-model="${escapeHTML(vehicle.modelName)}">
      ...
      <div class="vl-card-copy">
        ...
        ${(vehicle.custom?.vehicleMakeName || vehicle.vehiclesMeta?.vehicleMakeName) ? `<p class="vl-card-make">${escapeHTML(vehicle.custom?.vehicleMakeName || vehicle.vehiclesMeta.vehicleMakeName)}</p>` : ""}
        ...
        <div class="vl-card-actions">
          <a href="vehicle-details.html?model=${encodeURIComponent(vehicle.modelName)}">Details</a>
          <button
            type="button"
...
grid.querySelectorAll(
  ...
  function bindEvents() {
    ...
    el("vlSpawnApplyBtn")?.addEventListener("click", onSpawnNameApply);
