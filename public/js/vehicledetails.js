...
const el = id => document.getElementById(id);
  ...
  async function saveMetaFieldEdit(field, value) {
    ...
    try {
      ...
      if (result.restored) {
        ...
        if (DISPLAY_FIELDS.includes(field)) {
          store.saveVehicle({ modelName: state.vehicle.modelName, custom: { [field]: null } }).catch(() => {});
        }
      } else {
        ...
        if (DISPLAY_FIELDS.includes(field)) {
          store.saveVehicle({ modelName: state.vehicle.modelName, custom: { [field]: result.editedValue } }).catch(() => {});
        }
      }
  ...
  async function loadFieldEdits(modelName) {
    ...
    try {
      ...
      // Mirror display-critical field edits to local store so library card stays in sync
      await syncDisplayFieldsToLocal(modelName, state.fieldEdits);
    } catch (error) {
      console.warn("Personal vehicle edits could not be loaded.", error);
    }
  }

  // Persist display-critical field edit overrides into the vehicle's local custom object
  // so the library card shows the same values as the details page.
  const DISPLAY_FIELDS = ["vehicleMakeName", "gameName"];
  async function syncDisplayFieldsToLocal(modelName, fieldEdits) {
    if (!store?.saveVehicle) return;
    const updates = {};
    for (const field of DISPLAY_FIELDS) {
      const edit = fieldEdits[field];
      if (edit?.editedValue !== undefined && edit.editedValue !== "") {
        updates[field] = edit.editedValue;
      } else {
        updates[field] = null; // cleared / restored
      }
    }
    // Only write if there's something meaningful to update
    if (!Object.values(updates).some(v => v !== null)) return;
    try {
      await store.saveVehicle({
        modelName,
        custom: Object.fromEntries(Object.entries(updates).filter(([, v]) => v !== null))
      });
    } catch (e) {
      console.warn("Could not sync field edits to local store.", e);
    }
  }
